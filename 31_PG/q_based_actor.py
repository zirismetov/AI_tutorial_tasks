...
class A2CAgent:
    def __init__(self, state_size, action_size):
        ...
        self.q_model = Model_Q(self.state_size, self.action_size, args.hidden_size).to(self.device)
        self.q_t_model = Model_Q(self.state_size, self.action_size, args.hidden_size).to(self.device)
    ...
    def replay(self):
        ...
        td_delta = r_t1 + (args.gamma * q_t1) - q_t0

        v_t = self.c_model.forward(s_t1).squeeze()
        A = td_delta - v_t

        a_all = self.a_model.forward(s_t1)
