...
class Model(torch.nn.Module):
    def __init__(self):
        ...
        self.classifier = torch.nn.Sequential(
            torch.nn.Linear(EMBEDDING_SIZE, len(labels_train)),
            torch.nn.Softmax(dim=-1)
        )

    def forward(self, x):
        z = self.encoder.forward(x.expand(-1, 3, 56, 56))
        z = z.view(-1, EMBEDDING_SIZE)
...
RUN_PATH = f'c_loss/{int(time.time())}'
    ...
    for data_loader in [data_loader_train, data_loader_test]: # just for classification example
        ...
        for z in np_zs:
            dists_to_centers = cdist(
                np.array(list(centers_by_classes.values())), #TODO change here
