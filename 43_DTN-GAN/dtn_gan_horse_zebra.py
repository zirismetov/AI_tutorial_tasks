...
class ModelE(torch.nn.Module): # Encoder
    def __init__(self):
        ...
        self.encoder = torchvision.models.vgg11_bn(pretrained=True)
...
class ModelD(torch.nn.Module):  # Discriminator
    def __init__(self):
        ...
        self.encoder.fc = torch.nn.Linear(in_features=self.encoder.fc.in_features, out_features=4)
    ...
    def forward(self, x):
        x_enc_flat = self.encoder.forward(x)
        # x_enc_flat = x_enc.squeeze()  # B, 64, 1, 1, => B, 64
        y_prim = self.softmax.forward(x_enc_flat    )
        return y_prim
    ...
    for epoch in range(0, 500):
        ...
        for x_s in tqdm(data_loader_source, desc=stage):
            ...
            if n == 0:
                ...
                loss_gang = -torch.mean(torch.log(y_g_s[:, 3] + 1e-8)) - torch.mean(torch.log(y_g_t[:, 2] + 1e-8))
